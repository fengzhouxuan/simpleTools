...
8 个工具的具体用法、典型场景、参数含义、常见坑。如果你只是想快速跑通，直接看每节的"典型流程"。

> 全局：`Cmd+1~9` 切工具，`Cmd+Enter` 跑当前工具的主操作，`Cmd+,` 打开设置（macOS 标准）。
>
> **CLI 模式**：6 个工具能从命令行跑（`simpleimage <cmd>`），见末节。元数据剥离 / 图集增量打包暂未提供 CLI。
...
- 如果你真需要，可以 GUI 跑一次，或者直接调用 Node API（`require('.../metadata-strip.cjs')` 等）
